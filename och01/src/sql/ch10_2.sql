--ex01.employee 테이블 구조 복사-> emp_sample,
...
rename emp_sample to emp_sample;
...
--ex02. dept_sample생성하여 my_dept_pk로 지정
...
--ex04.사원테이블의 커미션 칼럼 0보다 큰 값만 입력
select * from emp_sample;
--ORA-02293: cannot validate (HR.EMP_COMMISSION_MIN) - check constraint violated
...
update emp_insert set emp_id = (select max(emp_id)+1 from emp_insert) 
   where emp_id = 7788 and rownum=1;--가상칼럼(pseudo)
alter table emp_insert add constraint emp_insert_pk primary key(emp_id);

select roenum, emp_id from emp_insert where emp_id=7788;
--중복 값 제거 후 primary key 제약조건 추가
alter table emp_insert add constraint emp_insert_pk primary key(emp_id);

--primary key 칼럼 두 개 이상 설정
create table emp_pr(
eno number not null,
ename varchar2(10),
sal number,
...
--두 개이상의 칼럼으로 primary key 생성 -복합키
alter table emp_pr add constraint my_emp_pr_pk primary key(eno,ename);
--칼럼이 두 개 이상 복합으로 키가 생성되면
--두 칼럼의 값으로 비교해서 키 중복 여부확인
