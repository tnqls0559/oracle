--제약조건 constraint
--칼럼이나 테이블에 조건을 지정하여 부정확한 데이터 입력 방지처리
--not null제약조건
...
--테이블 생성 시 not null제약조건으로 선언된 칼럼은 비어두면 안됨.
...
--not null제약조건이 추가돤 칼럼에는 반드시 값이 저장되어야함.
...
--ORA-00001: unique constraint (HR.SYS_C007108) violated
insert into customer3 values('hong','김길동','1111',null,'서울시');

--제약조건조회
--unique 제약조건을 선언하면 해당칼럼으로 index가 생성됨
...
--ORA-00001: unique constraint (HR.SYS_C007111) violated
insert into customer4 values('kim','홍길동','3333',null,null);
--unique 제약조건은 not null 제약조건이 아니므로 null값 중복 입력됨.
...
--froeign key 제약조건:부모테이블의 기본키가 자식테이블의 속성으로 참조되는 조건
select * from dept;
create table emp_f(
...
job varchar2(10),
dno number(9),
--constraint 제약조건명 foreign key(칼럼) references부모테이블(칼럼)
...
--참조하는 부모테이블의 칼럼의 값들(domain) 내의 값만 입력, 혹은 null 값 입력
insert into emp_f values(7788,'홍길동','Manager',10);
insert into emp_f values(7789,'임꺽정','Clerk',null);--null입력가능
insert into emp_f values(7790,'일지매','Clerk',40);
--참조무결성 제약조건 위배 (부모테이블에 해당 칼럼값 50이 존재하지 않음) 
--50대신 null 입력하거나 부모테이블인 department에 50을 추가 해주면 가능
...
--check제약도건: 값의 범위 지정, 지정된 범위의 값 만 등록 되도록 제약.
create table emp_c(
...
--check제약조건 범위를 벗어난 값 입력 시 오류 및 입력불가. 
insert into emp_c values(7789,'임꺽정',-1500);--check제약조건 -는 입력불가
...
select * from user_constraint where table_name='DEPT';
--참조되는 테이블(dept)의 기본키 생성(index생성)c
--참조하는 체이블(emp_copy)에서 참조되는 테이블(dept)관계 제약조건 추가
...
--참조 제약조건 비활성화 alter table 테이블명 disable constraint 제약조건며ㅇ;
alter table emp_copy disable constraint emp_copy_dno_fk;
--참조 제약조건 비활성화 후 부모테이블의 칼럼 삭제
delete from dept where dno=10;
--참조 제약조건 활성화 alter table 테이블명 enable constraint 제약조건명
--ORA-02430: cannot enable constraint (EMP_COPY_DNO_FK) - no such constraint
--비활성화 부모테이블의 primary key인 dno칼럼의 dno값이 변경,10이 사라짐.
alter table emp_copy enable constraint emp_copy_dno_fk;
select * from emp_copy;
select * from dept;
--사라진 dno 10 값을 다시 입력후 
